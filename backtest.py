...
ticker_data.append(yf.Ticker("PG"))
ticker_data.append(yf.Ticker("HD"))
ticker_data.append(yf.Ticker("WMT"))
...
for ticker in ticker_data:
    ...
    for index, row in data.iterrows():
        ...
        if not position and row['EMA5']>row['EMA20'] and tradeBlock>=15:
            ...
            positionSize = ((accountValue*risk) / 100) / (entryPrice-stopPrice)
        ...
        if position and row['EMA5']<row['EMA20'] and row['Close'] > entryPrice:
            position = False
            exitPrice = row['Close']
            wins += 1
